...
class ShopCardSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        for item_data in items_data:
            ...
            if item_id:
                item = BasketItem.objects.get(id=item_id, shop_card=instance)
                item.quantity = item_data.get('quantity', item.quantity)
                item.save()
            else:
                BasketItem.objects.create(shop_card=instance, **item_data)
        return instance
...
class BasketItemSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        representation = super().to_representation(instance)
        representation['product'] = representation.pop('product_name')
        return representation
...
class BasketSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        for item_data in items_data:
            item_id = item_data.get('id', None)
            if item_id:
                item = BasketItem.objects.get(id=item_id, basket=instance)
                item.quantity = item_data.get('quantity', item.quantity)
                item.save()
            else:
